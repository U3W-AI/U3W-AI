...
-- 变更日期：2026-04-10
-- 变更类型：新增菜单记录
-- 变更内容：FBS企业侧菜单 + 按钮权限写入 sys_menu 表
-- 影响范围：sys_menu 表（INSERT 操作）
-- 前置条件：FBS 父菜单 'FBS运营管理' 必须已存在
--          请先执行 update_20260409_平台场景包运营菜单.sql
-- 回滚方式：执行 DELETE FROM sys_menu WHERE menu_name IN ('企业管理','企业详情'...)
...
BEGIN
    IF @fbs_parent_id IS NULL THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'FBS父菜单(FBS运营管理)不存在，请先执行平台场景包运营菜单脚本';
    END IF;
END$$
DELIMITER ;
