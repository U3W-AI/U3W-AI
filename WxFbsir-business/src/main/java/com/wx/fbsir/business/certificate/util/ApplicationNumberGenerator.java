...
/**
 * 申请编号生成器
 * 格式：APP + YYYYMMDD + 6位递增序号（例如：APP20260110000001）
 */
@Component
public class ApplicationNumberGenerator {
    ...
    private static ApplicationNumberGenerator instance; // 单例实例
     ...
     * 获取当前实例（用于静态方法访问）
     */
    private static ApplicationNumberGenerator getInstance() {
        return instance;
    }
    ...
    public static String generateApplicationNumber() {
        ApplicationNumberGenerator generator = getInstance();
        if (generator == null) {
        ...
        return generator.generateUniqueApplicationNumber();
    }
    
    /**
     * 生成唯一的申请编号
     */
    private synchronized String generateUniqueApplicationNumber() {
        String dateStr = LocalDateTime.now().format(DATE_FORMATTER);
    ...
    private long getCurrentMaxSequenceForDate(String dateStr) {
        ...
        try {
            // 从数据库查询当天的最大序号
            maxSeq = applicationMapper.selectMaxSequenceForDate(dateStr, PREFIX);
        } catch (Exception e) {
            // 如果查询失败，从0开始
            maxSeq = 0;
        }
