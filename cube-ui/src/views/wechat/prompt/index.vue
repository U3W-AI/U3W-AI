...
import { getScorePromptList,getScorePrompt,deleteScorePrompt,saveScorePrompt,updateScorePrompt } from "@/api/wechat/aigc";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getScorePromptList(this.queryParams).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateScorePrompt(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            saveScorePrompt(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除评分模板配置编号为"' + ids + '"的数据项？').then(function() {
