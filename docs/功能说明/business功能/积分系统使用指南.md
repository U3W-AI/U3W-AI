# 积分系统使用指南

本文档说明如何在业务代码中使用积分系统进行积分扣减和发放。

---

## 📋 目录

- [快速开始](#快速开始)
- [积分前置校验服务](#积分前置校验服务)
- [使用示例](#使用示例)
- [错误码说明](#错误码说明)
- [注意事项](#注意事项)

---

## 快速开始

### 1. 核心概念

积分系统采用**前置校验**机制，在执行业务逻辑前先检查并扣减积分，确保：
- 积分余额充足
- 符合限频和累计上限规则
- 事务安全（失败时自动回滚）

### 2. 依赖注入

在需要使用积分功能的 Controller 或 Service 中注入 `PointsPrecheckService`：

```java
@Autowired
private PointsPrecheckService pointsPrecheckService;
```

### 3. 积分扣减使用步骤

#### 步骤1：确定积分规则编码

首先，需要确定业务对应的积分规则编码。可以在数据库 `wx_points_rule` 表中查询，或参考本文档[常见规则编码](#常见规则编码)部分。

**积分扣减规则示例**：
- `USE_DAILY_ASSISTANT`：使用日更助手（扣1积分）
- `ARTICLE_LAYOUT`：智能排版（扣1积分）
- `TEMPLATE_BUY`：模板购买

#### 步骤2：获取当前用户ID

通常从请求上下文中获取当前登录用户ID：

```java
Long userId = getUserId(); // 从BaseController继承的方法
```

#### 步骤3：调用积分前置校验服务

在业务逻辑执行**之前**调用积分前置校验服务：

```java
// 参数说明：用户ID、规则编码、自定义积分值（null则使用规则默认值）
PointsResult pointsResult = pointsPrecheckService.tryChangePoints(userId, "ARTICLE_LAYOUT", null);
```

#### 步骤4：处理校验结果

检查积分校验结果，如果失败则直接返回错误：

```java
if (!pointsResult.isSuccess()) {
    // 积分不足或规则校验失败，直接拦截，不执行后续业务
    return AjaxResult.error(pointsResult.getMsg());
}
// 积分校验通过，继续执行业务逻辑
```

#### 步骤5：业务完成后返回结果（可选）

可以将扣减后的积分余额返回给前端，方便用户了解当前积分情况：

```java
// 返回结果中包含扣减后的积分余额
Map<String, Object> result = new HashMap<>();
result.put("businessData", businessResult);
result.put("pointsBalance", pointsResult.getBalanceAfter()); // 扣减后的积分余额
return AjaxResult.success(result);
```

### 4. 完整积分扣减示例（智能排版功能）

```java
@PostMapping("/layoutArticle")
public AjaxResult layoutArticle(@RequestBody Map<String, Object> params) {
    try {
        // 1. 获取请求参数
        String content = params.get("content").toString();
        if (StringUtils.isEmpty(content)) {
            return AjaxResult.error("排版内容不能为空");
        }

        // 2. 获取当前用户ID
        Long userId = getUserId();
        
        // 3. 积分前置校验：智能排版扣减1积分
        PointsResult pointsResult = pointsPrecheckService.tryChangePoints(
            userId, 
            "ARTICLE_LAYOUT",  // 智能排版规则编码
            null                // 使用规则默认积分值（-1）
        );

        if (!pointsResult.isSuccess()) {
            // 积分不足或规则校验失败，直接拦截
            return AjaxResult.error(pointsResult.getMsg());
        }
        // ========== 积分校验通过，继续执行业务 ==========

        // 4. 执行智能排版业务逻辑
        String layoutedContent = dailyArticleService.layoutArticleSync(userId, content);

        // 5. 返回结果，包含排版内容和扣减后的积分余额
        Map<String, Object> result = new HashMap<>();
        result.put("layoutedContent", layoutedContent);
        result.put("message", "排版完成");
        result.put("pointsBalance", pointsResult.getBalanceAfter()); // 扣减后的积分余额

        return AjaxResult.success(result);

    } catch (Exception e) {
        log.error("排版失败: {}", e.getMessage());
        return AjaxResult.error("排版失败：" + e.getMessage());
    }
}
```

---

## 积分前置校验服务

### PointsPrecheckService

**核心方法**：`tryChangePoints(Long userId, String ruleCode, Integer changeAmount)`

**参数说明**：
- `userId`：用户ID（必填）
- `ruleCode`：规则编码（必填），使用枚举 `PointsRuleCode` 中的编码
- `changeAmount`：积分变动值（可选），不传则使用规则默认值

**返回值**：`PointsResult`
- `isSuccess()`：是否成功
- `getCode()`：错误码（失败时）
- `getMsg()`：错误消息（失败时）
- `getDelta()`：积分变动值（成功时）
- `getBalanceAfter()`：变动后余额（成功时）

---

## 使用示例

### 示例1：日更助手文章生成（扣减积分）

```java
@PostMapping("/createAndOptimize")
public AjaxResult createAndOptimize(@RequestBody Map<String, String> params) {
    Long userId = getUserId();
    
    // 积分前置校验：使用日更助手规则编码
    PointsResult pointsResult = pointsPrecheckService.tryChangePoints(
        userId, 
        PointsRuleCode.USE_DAILY_ASSISTANT.getCode(), 
        null
    );
    
    if (!pointsResult.isSuccess()) {
        // 积分不足或规则校验失败，直接拦截
        return AjaxResult.error(pointsResult.getMsg());
    }
    
    // 积分校验通过，继续执行业务
    DailyArticle article = dailyArticleService.createArticleAndOptimize(
        userId, 
        articleTitle, 
        selectedModels
    );
    
    // 返回结果，包含扣减后的积分余额
    Map<String, Object> result = new HashMap<>();
    result.put("id", article.getId());
    result.put("pointsBalance", pointsResult.getBalanceAfter());
    
    return AjaxResult.success(result);
}
```

### 示例2：每日登录奖励（增加积分）

```java
@PostMapping("/dailyLogin")
public AjaxResult dailyLogin() {
    Long userId = getUserId();
    
    // 积分前置校验：使用每日登录规则编码
    PointsResult pointsResult = pointsPrecheckService.tryChangePoints(
        userId, 
        PointsRuleCode.DAILY_LOGIN.getCode(), 
        null
    );
    
    if (!pointsResult.isSuccess()) {
        return AjaxResult.error(pointsResult.getMsg());
    }
    
    // 登录奖励发放成功
    return AjaxResult.success("登录奖励已发放，获得 " + pointsResult.getDelta() + " 积分");
}
```

### 示例3：自定义积分值

```java
// 如果需要在规则默认值基础上自定义积分值
PointsResult pointsResult = pointsPrecheckService.tryChangePoints(
    userId, 
    "USE_DAILY_ASSISTANT", 
    -5  // 自定义扣减5积分
);
```

---

## 错误码说明

| 错误码 | 说明 | 处理建议 |
|--------|------|---------|
| `USER_EMPTY` | 用户ID为空 | 检查用户登录状态 |
| `USER_NOT_FOUND` | 用户不存在 | 检查用户ID是否正确 |
| `USER_DISABLED` | 用户不可用 | 联系管理员启用用户 |
| `RULE_EMPTY` | 规则编码为空 | 检查规则编码参数 |
| `RULE_NOT_FOUND` | 积分规则不存在 | 检查规则编码是否正确，或联系管理员配置规则 |
| `RULE_DISABLED` | 积分规则已停用 | 联系管理员启用规则 |
| `POINTS_ZERO` | 积分变动值无效 | 检查规则配置 |
| `LIMIT_REACHED` | 已达到限频上限 | 提示用户稍后再试 |
| `MAX_AMOUNT_REACHED` | 已达到累计上限 | 提示用户已达到上限 |
| `INSUFFICIENT_BALANCE` | 积分余额不足 | 提示用户充值或获取积分 |
| `POINTS_ERROR` | 积分处理失败 | 记录日志，联系技术支持 |

---

## 注意事项

### 1. 规则编码使用

- ✅ **正确**：使用枚举 `PointsRuleCode.USE_DAILY_ASSISTANT.getCode()`
- ✅ **正确**：直接使用字符串 `"USE_DAILY_ASSISTANT"`
- ❌ **错误**：使用规则名称 `"使用日更助手"`

### 2. 业务执行顺序

**必须**在业务逻辑执行前进行积分校验：

```java
// ✅ 正确：先校验积分，再执行业务
PointsResult result = pointsPrecheckService.tryChangePoints(userId, ruleCode, null);
if (!result.isSuccess()) {
    return AjaxResult.error(result.getMsg());
}
// 执行业务逻辑
doBusiness();

// ❌ 错误：先执行业务，再校验积分
doBusiness();
PointsResult result = pointsPrecheckService.tryChangePoints(userId, ruleCode, null);
```

### 3. 事务处理

积分前置校验服务内部已包含事务处理，无需在业务层再次添加事务注解。

### 4. 异常处理

积分前置校验服务已统一封装所有异常情况，业务方只需检查 `isSuccess()` 即可。

---

## 常见规则编码

| 规则编码 | 说明 | 积分值 |
|---------|------|--------|
| `USE_DAILY_ASSISTANT` | 使用日更助手 | -1 |
| `ARTICLE_LAYOUT` | 智能排版 | -1 |
| `DAILY_LOGIN` | 每日登录 | +10 |
| `TEMPLATE_PUBLISH` | 模板上架 | +50 |
| `TEMPLATE_BUY` | 模板购买 | -10 |
| `TEMPLATE_REWARD` | 模板分成 | +8 |
| `RECHARGE` | 充值赠送 | +100 |
| `ADMIN_GRANT` | 管理员发放（历史退役，仅兼容既有记录） | 不再可用 |

---

## 权限管理

### 功能菜单权限

积分系统包含以下功能菜单及其权限配置：

| 菜单名称 | 菜单ID | 权限标识 | 说明 |
|---------|-------|---------|------|
| 积分管理 | 6 | - | 一级菜单 |
| 积分总览 | 120 | business:points:view | 积分总览页面 |
| 积分规则配置 | 121 | points:rule:list | 积分规则配置页面 |
| 粉丝管理 | 122 | points:fans:list | 粉丝管理页面 |

### 按钮权限

| 按钮名称 | 按钮ID | 所属菜单 | 权限标识 | 说明 |
|---------|-------|---------|---------|------|
| 积分查询 | 1070 | 积分总览 | business:points:query | 查询用户积分余额 |
| 明细查询 | 1071 | 积分总览 | business:points:record:query | 查询积分变动明细 |
| 规则查询 | 1072 | 积分规则配置 | points:rule:query | 查询积分规则 |
| 规则新增 | 1073 | 积分规则配置 | points:rule:add | 新增积分规则 |
| 规则修改 | 1074 | 积分规则配置 | points:rule:edit | 修改积分规则 |
| 规则删除 | 1075 | 积分规则配置 | points:rule:remove | 删除积分规则 |
| 粉丝列表 | 1076 | 粉丝管理 | points:fans:list | 仅全局 `admin`/`manager` 且具备细权限可查看粉丝 PII 列表 |
| 发放积分（已退役） | 1077 | 粉丝管理 | points:fans:grant | 旧权限标识仅为迁移存量；HTTP 写入口固定返回 410 |
| 查看明细 | 1078 | 粉丝管理 | points:fans:detail | 仅全局 `admin`/`manager` 且具备细权限可查看指定用户明细 |

### 角色权限分配

| 角色ID | 角色权限说明 |
|-------|------------|
| 2（`manager`） | 保留粉丝 PII 列表、指定用户明细的全局管理读取能力，但仍须分别具备原细权限；存量 `points:fans:grant` 不再提供写能力 |
| 3（`readonly`）及其他非全局管理角色（如 `operator`） | 可保留积分菜单分配，但即使误配粉丝细权限也不能读取粉丝 PII 列表或指定用户明细 |
| 10（普通用户） | 仅可访问自己的积分余额与明细；存量粉丝权限分配不再突破全局管理角色门禁 |

---

> 安全边界（2026-07-22）：`POST /points/fans/grantPoints` 与 `ADMIN_GRANT` 已历史退役；任意已认证身份访问旧入口均固定返回 HTTP 410 `POINTS_ADMIN_GRANT_RETIRED_USE_CREDIT_LEDGER`，匿名请求仍由全局 JWT 链返回 401，且不会调用旧积分服务。粉丝 PII 读取必须同时满足全局 `admin`/`manager` 角色与原细权限；`readonly`、`operator` 即使误配细权限也拒绝。粉丝列表查询统一经 `ISysUserService.selectUserList` 的 `@DataScope` 代理路径，切面会先清空请求携带的 `params.dataScope`，再生成可信角色范围，禁止客户端 SQL 片段直达 Mapper。旧粉丝管理页已移除写按钮。独董会管理员调整使用默认关闭的 `USER_GLOBAL/FBS_POINTS` 影子账本候选；在权限菜单、UI、部署门禁及内部消费写者迁移完成前，它既不可操作，也不得宣称为全局积分权威真源。

**最后更新**: 2026-07-22

