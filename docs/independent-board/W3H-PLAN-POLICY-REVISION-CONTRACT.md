...
边界：名称去首尾空白后为 1..128 个非控制字符；日会议额度为 1..10000，议题额度为 1..30，有限席位额度为 1..100。后两项上限与既有会议请求安全合同一致，不能保存用户链无法消费的幽灵权益。完整候选目录必须满足 VIP 的会议、议题和有限席位额度不低于 FREE；若 FREE 开启秘书能力，VIP 也必须开启。无实质变化的命令拒绝生成回执。
