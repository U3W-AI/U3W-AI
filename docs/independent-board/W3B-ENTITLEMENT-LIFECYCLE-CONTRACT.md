...
适用范围：W3b 套餐/配额策略当前读、受控权益撤销、不可变权益回执审计和撤销状态回读。
...
接口：`GET /business/independent-board/plans`

- 只允许同时具备 `admin` 角色与 `board:entitlement:query` 权限的全局管理员访问。该目录是既有权益授予表单的前置读合同，因此本波次复用权益查询权限，不新增未迁移、不可达的权限标识。
- 查询必须按服务端权威产品代码 `FBSIR_INDEPENDENT_BOARD` 限定范围，稳定按 `planCode` 排序，并额外读取第 3 行作为漂移哨兵；不得只过滤 `ACTIVE` 后掩盖禁用或多余策略。
- 当前目录必须且只能包含 `BOARD_FREE`、`BOARD_VIP` 两条已知策略。缺失、重复、多余、禁用、能力值漂移、无版本或无更新时间均以 500 失败关闭，不能把部分目录返回给管理台。
- 每条 DTO 只允许以下字段：
...
- 前端必须严格校验字段集合、产品代码、两个套餐的能力语义与版本元数据，并从该目录生成授予选项；目录失败或漂移时仍可只读查看既有权益，但必须停止新增和调整。
- 本接口不开放写入。后续套餐编辑必须先设计不可变策略修订、CAS、审计回执和迁移合同，不能直接修改当前行。
