...
#### Controller 层（放在 WxFbsir-business 模块）
```java
@RestController
...
#### Service 层（放在 WxFbsir-business 模块）
```java
@Service
...
**工具类路径：** `WxFbsir-common/src/main/java/com/wx/fbsir/common/utils/AesEncryptUtils.java`
...
1. **前端权限控制**
   ...
   - ✅ 页面组件放在 `WxFbsir-ui/src/views/business/` 目录下
   - ✅ API 文件放在 `WxFbsir-ui/src/api/business/` 目录下

2. **后端权限控制**
   - ✅ 所有 Controller 接口必须添加 `@PreAuthorize` 注解
   - ✅ 业务代码放在 `WxFbsir-business/src/main/java/com/wx/fbsir/business/` 包下
