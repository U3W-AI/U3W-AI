...
test('W1E admin receipt readback is GET-only, no-store, permission-bound and binding-safe', () => {
  const adminService = fs.readFileSync('FBSir-business/src/main/java/com/wx/fbsir/business/board/attribution/service/IndependentBoardAttributionAdminReadService.java', 'utf8')
  const adminController = fs.readFileSync('FBSir-business/src/main/java/com/wx/fbsir/business/board/attribution/controller/IndependentBoardAttributionAdminController.java', 'utf8')
  const noStoreFilter = fs.readFileSync('FBSir-business/src/main/java/com/wx/fbsir/business/board/attribution/controller/IndependentBoardAttributionNoStoreFilter.java', 'utf8')
  assert.match(adminService, /Pattern\.compile\("\[0-9a-f\]\{64\}"\)/)
  assert.match(adminService, /@Transactional\(readOnly = true\)\s+public Receipt receipt\(String requestedEventId\)/)
  assert.match(adminService, /mapper\.selectEventByEventId\(eventId\)/)
  assert.match(adminService, /sameBindingFingerprint\(row\.getSameBindingKey\(\)\)/)
  assert.match(adminService, /public record Receipt\([\s\S]*sameBindingFingerprint[\s\S]*eventWatermark\)/)
  const receiptRecord = adminService.slice(adminService.indexOf('public record Receipt('))
  assert.doesNotMatch(receiptRecord, /sameBindingKey|serverBindingId|journeyId|tenantSubjectDigest|signatureHex|nonceHash|traceparent/)
  assert.match(adminController, /@GetMapping\(path = "\/receipt", produces = "application\/json"\)/)
  assert.match(adminController, /@ss\.hasRole\('admin'\) and @ss\.hasPermi\('board:attribution:query'\)/)
  assert.match(noStoreFilter, /"\/business\/independent-board\/attribution\/receipt"/)
})
