...
Require ($reviewCandidate.source.commit -ceq '503d3de60d8b9e63f4db55c11f68adb363ac8a74') 'API2 review candidate commit drifted'
...
Require ($reviewCandidate.reviewBuild.recommendedSubmissionZip.path -ceq 'dist/review/fbs-connector-26.7.4-R.zip') 'API2 review candidate submission ZIP drifted'
Require ($reviewCandidate.reviewBuild.connectorPackageVersion -ceq '26.7.4') 'API2 review candidate connector version drifted'
Require ($reviewCandidate.reviewBuild.staticProductIdentityHeaders -ceq 'absent') 'API2 review candidate carries a stale product identity header'
...
Require ($reviewCandidate.releaseBoundary.productionReleaseCommit -ceq [string]$status.w1a.api2SourceTruth.reviewCandidateProductionBaselineCommit) 'API2 review candidate production baseline commit drifted'
...
Require (-not [string]::IsNullOrWhiteSpace($probeReleaseAppliedPath)) 'status has no API2 probe release-applied receipt'
Require ([string]$contract.artifacts.w1hApi2ProbeReleaseAppliedReceipt -ceq $probeReleaseAppliedPath) 'contract/status API2 probe release-applied receipt path drifted'
Require ([string]$w1Wave[0].api2ProbeReleaseAppliedReceipt -ceq $probeReleaseAppliedPath) 'taskboard/status API2 probe release-applied receipt path drifted'
$probeReleaseApplied = Read-Json $probeReleaseAppliedPath
Require ($probeReleaseApplied.schema -ceq 'fbsir.independentBoardApi2ProbeReleaseApplied.v1') 'API2 probe release-applied schema mismatch'
Require ($probeReleaseApplied.status -ceq 'DEPLOYED_RECORD_ONLY_PRODUCT_CREDIT_BLOCKED') 'API2 probe release-applied status drifted'
Require ($probeReleaseApplied.scope -ceq 'fbs_service_side') 'API2 probe release-applied owner surface drifted'
Require ($probeReleaseApplied.source.commit -ceq $probeReleaseGatePlan.source.commit) 'API2 probe release-applied source commit drifted'
Require ($probeReleaseApplied.deployment.status -ceq 'applied') 'API2 probe release was not applied'
Require ($probeReleaseApplied.deployment.cutoverTerminalStatus -ceq 'completed') 'API2 probe release cutover terminal status drifted'
Require ($probeReleaseApplied.deployment.rollbackTriggered -eq $false) 'API2 probe release rollback state drifted'
Require ($probeReleaseApplied.deployment.postDeployReadonlyPreflight -ceq 'passed') 'API2 probe release readonly preflight drifted'
Require ($probeReleaseApplied.fourProof.executableDeploymentChannel -ceq 'verified_and_executed') 'API2 probe release executable-channel proof drifted'
Require ($probeReleaseApplied.fourProof.targetScopedAuthorizationReceipt -ceq 'operator_invocation_with_user_authorized_service_release') 'API2 probe release target authorization proof drifted'
Require ($probeReleaseApplied.fourProof.exactRollbackAnchor -ceq 'previous_current_target_and_anchor_sha_readbacked') 'API2 probe release rollback proof drifted'
Require ($probeReleaseApplied.fourProof.targetSideCommitBuildDigestReadback -ceq 'release_health_and_cutover_receipt_three_digests_match') 'API2 probe release target digest proof drifted'
Require ($probeReleaseApplied.targetReadback.sourceGitHead -ceq $probeReleaseApplied.source.commit) 'API2 probe release target source readback drifted'
Require ($probeReleaseApplied.targetReadback.packageManifestSha256 -ceq $probeReleaseApplied.source.packageManifestSha256) 'API2 probe release target manifest readback drifted'
Require ($probeReleaseApplied.targetReadback.criticalDeploySnapshotSha256 -ceq $probeReleaseApplied.source.criticalDeploySnapshotSha256) 'API2 probe release target snapshot readback drifted'
Require ($probeReleaseApplied.releaseBoundary.officialExpertsPackageChanged -eq $false) 'probe release applied candidate changed the official experts package'
Require ($probeReleaseApplied.releaseBoundary.hostChanged -eq $false) 'probe release applied candidate changed the host'
Require ($probeReleaseApplied.releaseBoundary.officialConnectorChanged -eq $false) 'probe release applied candidate changed the official connector'
Require ($probeReleaseApplied.releaseBoundary.naturalSameBindingObserved -eq $false) 'probe release applied candidate promoted natural same-binding'
Require ($probeReleaseApplied.releaseBoundary.productCreditPromotion -ceq 'blocked') 'probe release applied candidate promoted product credit'
Require ([string]$status.w1a.api2SourceTruth.probeContractCandidate.commit -ceq [string]$probeReleaseApplied.source.commit) 'status probe release-applied candidate commit drifted'
