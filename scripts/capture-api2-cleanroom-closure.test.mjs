...
test('captures manifest consistency, runtime inputs, and per-file critical inventory evidence', () => {
  ...
  try {
    ...
    assert.ok(report.runtimeInputs.optional.some(item => item.path === 'reports/api2-admin-test-account-dryrun-latest.json' && !item.exists))
    assert.equal(report.runtimeInputs.specCoverage.declaredServePathCount, 25)
    assert.equal(report.runtimeInputs.specCoverage.capturedSpecCount, 25)
