...
- [第三阶段：元器工作流配置](#第三阶段元器工作流配置)
  ...
  - [9. 工作流导入与配置](#9-工作流导入与配置)
    ...
    - [9.9 文档解析助手工作流文件说明](#99-文档解析助手工作流文件说明)
    - [9.10 部署TikaMcp工具包](#910-部署tikamcp工具包)
    - [9.11 导入文档解析工作流](#911-导入文档解析工作流)
    - [9.12 修改HTTP回调节点配置](#912-修改http回调节点配置-1)
    - [9.13 测试工作流](#913-测试工作流-1)
    - [9.14 发布工作流](#914-发布工作流-1)
    - [9.15 获取工作流配置信息](#915-获取工作流配置信息-1)
    - [9.16 修改内网穿透域名配置](#916-修改内网穿透域名配置)
    - [9.17 在系统中配置智能体](#917-在系统中配置智能体-1)
- [第四阶段：测试验证](#第四阶段测试验证)
  - [10. 功能测试](#10-功能测试)
    - [10.1 测试日更助手](#101-测试日更助手)
    - [10.2 测试排版功能（如果配置了排版助手）](#102-测试排版功能如果配置了排版助手)
    - [10.3 测试文档解析助手](#103-测试文档解析助手)
    - [10.4 完成部署验证](#104-完成部署验证)
- [常见问题](#常见问题)
- [技术支持](#技术支持)
...
项目提供了文档解析助手工作流配置文件（位于 `docs/workflows/` 目录）：

**[export-文档解析工作流.zip](docs/workflows/export-%E6%96%87%E6%A1%A3%E8%A7%A3%E6%9E%90%E5%8A%A9%E6%89%8B.zip)**
- 功能：文档解析助手核心工作流
- 用途：接收文档URL和提示词，调用Tika解析文档内容，根据提示词提取关键信息

### 9.10 部署TikaMcp工具包

文档解析助手依赖Apache Tika进行文档解析，需要先部署TikaMcp工具包。

#### 步骤1：解压工具包
```bash
unzip docs/workflows/TikaMcp.zip
cd TikaMcp
```
...
#### 需要修改的节点
`文档解析助手`工作流中有 **1个HTTP请求节点** 需要修改：
- 节点名称可能是"保存解析内容"或类似名称
...
3. **修改请求URL**
   - 找到"请求URL"配置项
   - 原始地址可能是：`http://localhost:8080/system/document-parse/updateParsedContent`
   - 修改为您的公网地址：`http://您的公网IP:端口/system/document-parse/updateParsedContent`

4. **保持其他配置不变**
   - 请求方法：POST
   - Content-Type：application/json
   - 请求体：保持原有配置（包含 documentParseId, parsedContent 等参数）

### 9.13 测试工作流

#### 步骤0：上传测试文档（重要）

**⚠️ 重要提示**：在向工作流发送测试JSON之前，必须先在本地通过 `/upload` 接口上传文档获取真实的文件URL，然后替换JSON中的url字段。

**原因**：如果使用虚假的URL或未上传的URL，MCP工具包可能无法解析下载链接，导致工作流测试失败。

**上传文档步骤**：

1. **使用 Postman 或 curl 上传文档**

   **Postman 请求配置**：
   - 方法：POST
   - URL：`http://localhost:8080/common/upload`
   - Headers：
     - Authorization: `Bearer {您的token}`（从登录接口获取）
   - Body：
     - 类型：form-data
     - Key：`file`（类型选择 File）
     - Value：选择要上传的测试文档（如 test.pdf）
...
2. **获取上传响应中的URL**
   ...
   ```json
   {
     "code": 200,
     "msg": "操作成功",
     "url": "http://localhost:8080/profile/upload/2026/01/12/test_xxx.pdf",
     "fileName": "test_xxx.pdf",
     "originalFilename": "test.pdf",
     "newFileName": "test_xxx.pdf"
   }
   ```

3. **替换URL中的域名**

   将返回的 `url` 字段中的 `localhost:8080` 替换为您的内网穿透域名：
   ```bash
   原始URL: http://localhost:8080/profile/upload/2026/01/12/test_xxx.pdf
   替换后: http://您的内网穿透域名.com/profile/upload/2026/01/12/test_xxx.pdf
   ```

4. **验证文件可访问**

   在浏览器中访问替换后的URL，确保可以正常下载文件：
   ```
   http://您的内网穿透域名.com/profile/upload/2026/01/12/test_xxx.pdf
   ```

#### 步骤1：使用工作流测试功能
1. 在工作流编辑页面，点击右上角"调试"按钮，然后点击"去调试"按钮
2. 输入测试参数（**注意：url字段必须使用步骤0中获取的真实URL**）：
   ```json
   {
    "assistant_id": "2009821745983628544",
    "user_id": "1",
    "stream": False,
    "messages": [
        {
            ...
            "content": [
                {
                    "type": "text",
                    "text": '{"documentId":"TEST-001","prompt":"分板块总结文档内容","url":"http://您的内网穿透域名.com/profile/upload/2026/01/12/test_xxx.pdf","originalFilename":"test.pdf"}'  
                }
            ]
        }
    ]
   ...
   ```
   **参数说明**：
   - `documentId`: 自定义文档ID（如 TEST-001）
   - `prompt`: 提示词（如"分板块总结文档内容"）
   - `url`: **必须使用步骤0中上传后获取的真实URL，并替换为内网穿透域名**
   - `originalFilename`: 原始文件名（如 test.pdf）
...
文档解析助手需要配置内网穿透域名，以便腾讯元器智能体能够访问上传的文档。
...
2. 找到 `wxfbsir` 配置部分，修改 `tunnel-domain` 配置项：
   ...
   wxfbsir:
     ...
     tunnel-domain: http://内网穿透域名
   ```

3. 将 `http://内网穿透域名` 替换为您的实际内网穿透域名
...
**Windows:**
```bash
set WXFBSIR_TUNNEL_DOMAIN=http://内网穿透域名
```

**Linux/Mac:**
```bash
export WXFBSIR_TUNNEL_DOMAIN=http://内网穿透域名
```
...
1. 进入"文档解析助手"页面
2. 点击"上传文档"按钮
3. 选择一个PDF或Word文档（支持格式：PDF、DOC、DOCX、TXT等）
4. 输入提示词，例如："请提取文档中的关键信息"
5. 点击"上传并解析"

#### 步骤2：等待解析
- 系统会显示"上传成功，AI正在解析文档"
- 文档状态显示为"处理中"
- 等待约1-2分钟（取决于文档大小和网络速度）
...
### Q8: 文档解析失败
**症状**：文档上传后一直显示"处理中"或显示"解析失败"

**排查步骤**：
1. 检查内网穿透是否正常：
```bash
curl http://您的公网IP:端口/profile/upload/xxx.pdf
```
...
5. 检查配置文件中的内网穿透域名：
   - 确认 `application.yml` 中 `wxfbsir.tunnel-domain` 已配置为实际的内网穿透域名
   - 重启后端服务使配置生效

### Q9: 文档上传成功但无法访问
**症状**：上传文档后，通过公网地址无法访问文档
...
# application.yml
wxfbsir:
  profile: D:/WxFbsir/uploadPath  # 确认路径正确
```
...
```bash
# Windows
dir D:\WxFbsir\uploadPath\profile\upload\2026\01\12

# Linux
ls -la /home/WxFbsir/uploadPath/profile/upload/2026/01/12
```

4. 检查文件权限（Linux）：
```bash
chmod 755 /home/WxFbsir/uploadPath/profile/upload/2026/01/12
```
