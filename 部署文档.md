...
文档更新日期：2026-07-11 文档版本：1.2.0

> 新部署统一使用 `FBSIR_*` 环境变量。历史 `WXFBSIR_*` 变量仍作为升级兼容别名保留；若两套变量同时存在，以 `FBSIR_*` 为准。数据库名 `wxfbsir` 是历史技术标识，品牌升级不会自动迁移现有数据库。

> 安全必填项：`FBSIR_TOKEN_SECRET` 至少 32 个字符；数据库密码不得依赖仓库默认值。Druid 控制台默认关闭，仅在同时配置管理用户名、密码并显式启用后开放。
...
### 4.1 配置数据库连接
编辑 `FBSir-admin/src/main/resources/application-druid.yml`：
```yaml
spring:
  datasource:
    druid:
...
### 4.2 配置 Redis
编辑 `FBSir-admin/src/main/resources/application.yml`：
```yaml
spring:
  redis:
    host: localhost
...
应该看到启动成功的日志：
```
欢迎使用福帮手后台管理框架，当前版本：v1.0.0，请通过前端地址访问。
```
...
编辑 `FBSir-engine/src/main/resources/application.yml`：
...
fbsir:
  engine:
    ...
    # 主机ID是连接主节点的唯一凭证，需要：
    # 1. 联系管理员申请主机ID
    # 2. 管理员在主节点数据库白名单表中添加此ID，并确保 hostType=engine
    # 3. 将申请到的主机ID填写到此处
    # ⚠️ 注意：
...
# 启动Engine服务
java -jar target/fbsir-engine-1.3.1.jar
```
...
- 然后参考[README.md](FBSir-engine%2FREADME.md)进行后续测试

**常见问题**：
- 如果连接失败，检查 `ws-url` 配置是否正确
- 如果提示"主机ID不在白名单"，确认数据库中是否已导入 `ws_host_whitelist` 表数据，并且该记录的 `hostType` 为 `engine`
...
```bash
# Windows
set FBSIR_DOMAIN=http://您的公网IP:端口

# Linux/Mac
export FBSIR_DOMAIN=http://您的公网IP:端口
```
